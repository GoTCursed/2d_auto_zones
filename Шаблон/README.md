...
На ступени детализации `Min` диапазон включает все ячейки с `As − фон > 0,01 см²/м`; сглаживание и разнесение зон не применяются, чтобы прямоугольники полностью сохраняли габариты расчётных пятен.
Все ещё не покрытые положительные ячейки рассматриваются как кандидаты после обработки пиков. У края плиты стандартная длина зоны сдвигается внутрь доступного габарита до подрезки.
...
# Дополнительно: -InputJson "C:\path\lira_all_plates.json" (проверка отметки +55.700, фон Ø12/200, B25)
```
